...
def binary(img, thresh_val, maxval, thresh_type=cv2.THRESH_BINARY,
           grayscale=True, display=False):
    """
    Binary threshold
    ...
    Also, the special values cv::THRESH_OTSU or cv::THRESH_TRIANGLE may be
    combined with one of the above values. In these cases, the function
    determines the optimal threshold value using the Otsu's or Triangle
    algorithm and uses it instead of the specified thresh . The function
    returns the computed threshold value. Currently, the Otsu's and Triangle
    methods are implemented only for 8-bit images.

    @note Input image should be single channel only in case of CV_THRESH_OTSU
    or CV_THRESH_TRIANGLE flags.
...
def adaptive(img, maxval, method=cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
    ...
    to the formulae:
    =>   **THRESH_BINARY**
            \f[dst(x,y) =  \fork{\texttt{maxValue}}{if \(src(x,y) > T(x,y)\)}
                                {0}{otherwise}\f]
    =>   **THRESH_BINARY_INV**
            \f[dst(x,y) =  \fork{0}{if \(src(x,y) > T(x,y)\)}
                                {\texttt{maxValue}}{otherwise}\f]
            where \f$T(x,y)\f$ is a threshold calculated individually for
                                each pixel (see adaptiveMethod parameter).
    ...
    :param maxval:
            Non-zero value assigned to the pixels for which the condition is
            satisfied
    :param method:
            Adaptive thresholding algorithm to use.
            see cv::AdaptiveThresholdTypes
    :param thresh_type:
            Thresholding type that must be either THRESH_BINARY or
